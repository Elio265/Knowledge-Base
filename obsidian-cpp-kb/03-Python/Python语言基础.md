...
- 可变默认参数陷阱：`def f(x, lst=[]): ...` 在**函数定义时**就创建空 list 作为默认对象，调用时**共享同一对象**；原地修改（`append` 等）会污染默认值，跨调用累积。改法：`lst=None` 哨兵 + 函数体内 `if lst is None: lst = []`；哨兵判断用 `is None`，不要用 `if lst:`（会吞掉空 list）或 `== []`（不必要的 `__eq__` 调用）。
- 浅拷贝只复制外层容器、内层元素是引用：`a[:]` / `list(a)` / `copy.copy()` 对嵌套可变对象不独立（"换信封不改信纸"）；`copy.deepcopy` 递归拷贝。对 list 而言 `a[:]` 和 `list(a)` 等价；后者走迭代器协议，适用范围更广（任意 iterable）。
- 深拷贝的边界：`copy.deepcopy` 对文件、线程锁、socket、C 扩展对象（含 OS 级或原生句柄的对象）会**报错或拷贝失真**——含这类资源的对象不要深拷贝。`tuple` 不可变但 deepcopy 仍新建外层，对嵌套可变对象递归拷贝。
- 循环引用必须用 memo：`copy.deepcopy` 通过 memo 字典（id → 副本映射）+ 先占位再递归来维护图同构；手写递归会无限递归导致 `RecursionError`。图同构 ≠ 值相等，而是"原图里指向同一对象的所有边，拷贝后都指向该对象对应的那一个副本"。
